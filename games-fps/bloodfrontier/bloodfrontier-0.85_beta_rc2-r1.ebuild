...
QA_PRESTRIPPED="${DIR:1}/bfclient_linux
		${DIR:1}/bfserver_linux"

src_install() {
	use amd64 && multilib_toolchain_setup x86

	exeinto "${DIR}"
	doexe bin/bfclient_linux || die "doexe failed"

	insinto "${DIR}"
	doins -r data || die "doins failed"

	games_make_wrapper bloodfrontier-client ./bfclient_linux "${DIR}" "${DIR}"

	if use server ; then
		doexe bin/bfserver_linux || die "doexe failed"
		games_make_wrapper bloodfrontier-server ./bfserver_linux "${DIR}" "${DIR}"
	fi
